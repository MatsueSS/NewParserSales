...
def click_copy_button(coords_x, coords_y):
    """Нажимает кнопку копирования через ydotool"""
    try:
        subprocess.run(['ydotool', 'mousemove', coords_x, coords_y], check=False)
        time.sleep(0.3)
...
def main():
    ...
    for i, item in enumerate(URLS, 1):
        ...
        click_copy_button('80', '75')

        click_copy_button('835', '25')
        ...
        if i < len(URLS):
            print(f"\n⏭️  Переход к следующей категории через 2 секунды...")
            time.sleep(2)
